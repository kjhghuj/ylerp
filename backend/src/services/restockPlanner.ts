...
/**
 * V3 计算策略（缺省 = V2 旧行为，V2 调用方不传 policies 行为完全不变）：
 * - missingStockPolicy：元仓未返回库存行时 'zero' 按零库存计算（旧）；'unknown' 视为未知库存，
 *   进入 no_stock_data 状态、不输出可执行建议量（补货V3）。
 * - inboundEtaPolicy：'optimistic' 无 ETA 在途计入到仓前库存、ETA==targetDate 计入覆盖期（V2 旧行为）；
 *   'strict' 无 ETA / 逾期不计入确定供应，且在途可用区间与需求区间一致为左闭右开
 *   [planningDate, targetDate)（ETA==targetDate 归入 afterCoverage）（补货V3）。
 * - simulateDaily：逐日模拟库存变化，输出 stockoutDate / gapBeforeArrival / stockSim。
 * - quantityMode：'formula' 建议量 = max(0, 覆盖期需求+安全库存−到仓库存−覆盖期在途)（V2 旧公式）；
 *   'simulation' 建议量由「不新增补货」基线轨迹推导（到仓后缺口 + 期末安全缺口），
 *   晚到在途不提前抵扣，与展示轨迹同源（补货V3）。
 */
export interface RestockPlanPolicies {
  missingStockPolicy?: 'zero' | 'unknown';
  inboundEtaPolicy?: 'optimistic' | 'strict';
  simulateDaily?: boolean;
  quantityMode?: 'formula' | 'simulation';
}
...
/** 在途明细分类（到仓前 / 覆盖期内 / 覆盖期后 / 无ETA / 已逾期） */
export type InboundCategory = 'beforeArrival' | 'duringCoverage' | 'afterCoverage' | 'noEta' | 'overdue';
...
export interface RestockPlanItem {
  ...
  /** strict 口径：无 ETA 的在途（不计入确定供应） */
  inTransitNoEta: number;
  /** strict 口径：ETA 早于计划日（逾期未入库，不计入确定供应） */
  inTransitOverdue: number;
  /** ETA 晚于需求区间（≥ targetDate，strict 口径；> targetDate，optimistic 口径）的在途 */
  ...
  reorderPoint: number;
  suggestedQty: number;
  /** 成本未知时为 null（不显示为零成本）；summary 只累计已知成本行 */
  ...
  /** 在途明细（单号 / 明细身份 / 剩余量 / ETA / 类别），可追溯 */
  inboundBreakdown: InboundBreakdownEntry[];
  /** 采用建议量后的库存轨迹（丢失销量假设） */
  stockSim: DailyStockPoint[] | null;
  /** 不新增补货时的库存轨迹（对照展示） */
  baselineStockSim: DailyStockPoint[] | null;
  /** 采用建议量后仍会断货的首日（通常为到仓前不可避免缺口）；无断货为 null */
  stockoutDate: string | null;
  /** 不新增补货时的首断货日（对照展示） */
  baselineStockoutDate: string | null;
  /** 本次补货到仓前的未满足需求（丢失销量假设，单独展示，不并入建议量） */
  gapBeforeArrival: number;
  /** 到仓后、确定在途到达前的缺口合计（由本次建议量覆盖；基线轨迹推导） */
  gapAfterArrival: number;
  /**
   * 基线（不新增补货）轨迹的期末安全缺口 = max(0, 安全库存目标 − 基线期末库存)。
   * 建议量等式第二项：suggestedQty = ceil(gapAfterArrival + baselineEndSafetyGap)。
   * 与 endSafetyGap（采用建议量后的剩余缺口）语义不同，不得混用。
   */
  baselineEndSafetyGap: number;
  /** 采用建议量后期末库存与安全库存目标的差额（正常为 0） */
  endSafetyGap: number;
  /** 建议量向上取整前的原始和（simulation 模式 = gapAfterArrival + baselineEndSafetyGap；formula 模式为 null） */
  suggestedQtyRaw: number | null;
  /** 库存与销量维度是否可执行（质量维度由调用方叠加） */
  executable: boolean;
  /** 参数继承来源 */
  ruleSources: RuleSources;
  /** 不可执行/待核对原因（如元仓未返回库存行）；正常行为 null */
...
export interface RestockPlan {
  ...
  summary: {
    ...
    /** 参与金额估算的 SKU 数（成本未知的行不计入 estimatedCost） */
...
export interface BuildRestockPlanInput {
  ...
  generatedAt?: string;
  policies?: RestockPlanPolicies;
  /** 真实零销量 SKU（有观测但区间件数合计为 0）：状态 zero_sales，区别于 missing_sales */
  zeroSalesSkus?: string[];
}
...
interface StockAggregate {
  ...
  warehouseName: string | null;
  availableStock: number;
  byWarehouse: Map<string, StockByWarehouse>;
}
...
interface InboundAggregate {
  ...
  warnings: string[];
  breakdown: InboundBreakdownEntry[];
}
...
const buildRemoteStockMap = (rows: RemoteStockRow[] | undefined, site: string): Map<string, StockAggregate> => {
  ...
  for (const row of rows || []) {
    ...
    const previous = result.get(sku) || {
      ...
      warehouseName: null,
      availableStock: 0,
      byWarehouse: new Map<string, StockByWarehouse>(),
    };
    ...
    warehouseEntry.available = safeAdd(
      warehouseEntry.available,
      available,
      `available for SKU ${sku} warehouse ${warehouseCode}`,
    );
...
const inboundOrderKey = (order: RemoteInboundOrder): string | null => {
  const warehouseOrderNo = String(order.warehouseOrderNo || '').trim();
  if (warehouseOrderNo) return `wo:${warehouseOrderNo}`;
  const customerNo = String(order.customerWarehouseOrderNo || '').trim();
  if (customerNo) return `cw:${customerNo}`;
  ...
  for (const order of orders || []) {
    ...
    for (const detail of order.details || []) {
      ...
      if (detailId !== null && detailId !== undefined && String(detailId) !== '') {
        ...
        if (seenDetailIds.has(detailKey)) continue;
...
const summarizeInbound = (
  ...
  const aggregate: InboundAggregate = {
    beforeArrival: 0,
    duringCoverage: 0,
    noEta: 0,
    overdue: 0,
  ...
  const addField = (field: 'beforeArrival' | 'duringCoverage' | 'noEta' | 'overdue' | 'afterCoverage', entry: InboundEntry) => {
  ...
  for (const entry of entries || []) {
    if (!entry.eta) {
      if (inboundEtaPolicy === 'optimistic') {
        ...
        aggregate.warnings.push(
          `Inbound order ${entry.orderNumber} for SKU ${sku} has no valid ETA; excluded from confirmed supply.`,
        );
      }
    ...
    if (entry.eta.getTime() < planningDate.getTime()) {
      if (inboundEtaPolicy === 'optimistic') {
        ...
        aggregate.warnings.push(
          `Inbound order ${entry.orderNumber} for SKU ${sku} has an overdue ETA (${formatDateOnly(entry.eta)}); excluded from confirmed supply.`,
        );
      }
      aggregate.breakdown.push({
        orderNumber: entry.orderNumber,
...
interface SimulationResult {
  sim: DailyStockPoint[] | null;
  stockoutDate: string | null;
  /** 新补货到仓日之前的未满足需求合计（丢失销量，无法用本次补货修复） */
  gapBeforeArrival: number;
  /** 到仓日（含）之后、需求区间结束前的未满足需求合计 */
  gapAfterArrival: number;
  /** 需求区间结束（targetDate 前一日收盘）时的库存 */
  endStock: number;
}
...
const runDailySimulation = (
  startStock: number,
  arrivalsByDate: Map<string, number>,
  dailyDemand: number,
  planningDateValue: Date,
  ...
  for (let day = 0; day < horizonDays; day += 1) {
    ...
    let dayEndStock: number;
    let stockout = false;
    if (availableToday < dailyDemand) {
      stockout = true;
      if (stockoutDate === null) stockoutDate = dateStr;
      const unserved = dailyDemand - Math.max(0, availableToday);
      if (date.getTime() < arrivalDateValue.getTime()) gapBeforeArrival += unserved;
      else gapAfterArrival += unserved;
      dayEndStock = 0;
    } else {
...
const buildArrivalsByDate = (
  entries: InboundEntry[] | undefined,
  planningDateValue: Date,
  targetDateValue: Date,
  inboundEtaPolicy: 'optimistic' | 'strict',
  extraAtArrival: number,
  arrivalDateValue: Date,
): Map<string, number> => {
  const arrivalsByDate = new Map<string, number>();
  for (const entry of entries || []) {
    if (!entry.eta) continue; // 无 ETA 无法定位到货日，不参与模拟
    if (inboundEtaPolicy === 'strict' && entry.eta.getTime() < planningDateValue.getTime()) continue;
    if (entry.eta.getTime() > targetDateValue.getTime()) continue;
...
const buildReason = (status: RestockStatus, daysCover: number, leadTimeDays: number, suggestedQty: number): string => {
  if (status === 'no_stock_data') return 'YC did not return a stock row for this SKU; quantity is not executable.';
  if (status === 'missing_sales') return 'Missing daily sales data';
  if (status === 'zero_sales') return 'Confirmed zero sales in the period; no replenishment needed.';
  if (status === 'critical') {
...
export const buildRestockPlan = ({
  ...
  const items = products
    ...
    .map((product): RestockPlanItem => {
      ...
      const growthSource: RuleSources['growthPercent'] = rule?.growthPercent !== undefined ? 'sku-rule' : 'global';
      ...
      const safetySource: RuleSources['safetyDays'] = rule?.safetyDays !== undefined ? 'sku-rule' : 'global';
      ...
      const leadTimeSource: RuleSources['leadTimeDays'] = rule?.leadTimeDays !== undefined
        ? 'sku-rule'
      ...
      const stockSource: StockSource = hasRemoteRow ? 'yc' : 'missing';
      ...
      let suggested: SimulationResult = baseline;
      let suggestedQty = 0;
      let arrivalStock = 0;
      let endSafetyGap = 0;
      let baselineEndSafetyGap = 0;
      let suggestedQtyRaw: number | null = null;

      if (useSimulation) {
        // 基线轨迹（不新增补货）：U=到仓后缺口、E0=期末库存
        baseline = runDailySimulation(
          availableStock,
          buildArrivalsByDate(
            inboundEntriesBySku.get(skuKey),
            planningDateValue,
          ...
          arrivalDateValue,
          planningHorizonDays,
          planningHorizonDays <= MAX_STOCK_SIM_DAYS,
        );
        ...
        const rawSuggestedQty = ensureSafeFinite(
          targetDemand - Math.max(0, stockBeforeTransportDemand - transportDemand) - inbound.duringCoverage,
      ...
      const status: RestockStatus = stockUnknown
        ? 'no_stock_data'
        : dailySales <= 0
      ...
      return {
        ...
        suggestedQty,
        estimatedCost: costUnknown ? null : suggestedQty * (unitCost as number),
        costUnknown,
        status,
        ...
        stockByWarehouse: remoteStock
          ? Array.from(remoteStock.byWarehouse.values()).sort((left, right) =>
  ...
  const summary = items.reduce(
    (acc, item) => {
      ...
      if (item.status === 'zero_sales') acc.zeroSalesCount += 1;
      if (item.status === 'no_stock_data') acc.noStockDataCount += 1;

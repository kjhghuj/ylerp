...
interface YcInboundBoxDetail {
  detail?: RemoteInboundDetail[] | null;
}

interface YcInboundOrderDetail extends Omit<RemoteInboundOrder, 'details'> {
  details?: Array<RemoteInboundDetail | YcInboundBoxDetail> | null;
}

export const YC_CLIENT_LIMITS = Object.freeze({
...
export class YcClientError extends Error {
  constructor(
    message: string,
...
export interface YcListProductInventoryParams {
  warehouseCodes?: string[];
  customerSkus?: string[];
}

export interface YcListInboundOrdersParams {
  warehouseCodes?: string[];
}
...
const compactBaseUrl = (baseUrl: string) => baseUrl.replace(/\/+$/, '');
...
const validateScope = (values: string[], maximum: number): string[] => {
  if (values.length > maximum) {
    throw new YcClientError('YC request scope is too large', 'SCOPE_LIMIT');
  }
  const normalized = values.map(value => String(value).trim());
  if (normalized.some(value => !value || value.length > YC_CLIENT_LIMITS.maxIdentifierLength)) {
...
const flattenInboundDetails = (
  details: YcInboundOrderDetail['details'],
): RemoteInboundDetail[] => {
  const flattened: RemoteInboundDetail[] = [];
  for (const entry of details || []) {
    if ('detail' in entry && Array.isArray(entry.detail)) {
      ...
      flattened.push(entry as RemoteInboundDetail);
    }
  }
  return flattened;
};
...
export class HttpYcOpenPlatformClient implements YcOpenPlatformClient {
  ...
  constructor(options: {
    ...
    this.requestTimeoutMs = Number.isFinite(requestedTimeout) && requestedTimeout >= 1 && requestedTimeout <= 120000
      ? requestedTimeout
      : YC_CLIENT_LIMITS.requestTimeoutMs;
  }
  ...
  async listCustomerWarehouses(): Promise<YcCustomerWarehouse[]> {
    const data = await this.request<YcCustomerWarehouse[]>('/api/openPlatform/baseData/customerWarehouse', {});
    const warehouses = Array.isArray(data) ? data : [];
    if (warehouses.length > YC_CLIENT_LIMITS.maxWarehouseCodes) {
      throw new YcClientError(
        'YC warehouse limit exceeded',
        'WAREHOUSE_LIMIT',
        '/api/openPlatform/baseData/customerWarehouse',
      );
    }
    return warehouses;
  }
  ...
  private async paginate<T>(path: string, body: Record<string, unknown>): Promise<T[]> {
    ...
    for (let page = 1; page <= MAX_PAGES; page += 1) {
      const data = await this.request<YcListResponse<T>>(path, {
        ...body,
        page,
      ...
      if (!Number.isFinite(total) || total < 0 || total > YC_CLIENT_LIMITS.maxListRows) {
        throw new YcClientError('YC list row limit exceeded', 'ROW_LIMIT', path);
      }
      if (rows.length + list.length > YC_CLIENT_LIMITS.maxListRows) {
        throw new YcClientError('YC list row limit exceeded', 'ROW_LIMIT', path);
      }
      rows.push(...list);
    ...
    if (!completed) throw new YcClientError('YC pagination limit reached', 'PAGE_LIMIT', path);
    return rows;
  }
  ...
  private async getToken(): Promise<TokenCache> {
    ...
    const data = await this.request<{ token?: string; tokenType?: string }>(
      '/api/openPlatform/authorization/login',
  ...
  private async request<T>(path: string, body: Record<string, unknown>, withAuth = true): Promise<T> {

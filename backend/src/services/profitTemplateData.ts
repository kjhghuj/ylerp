...
export class ProfitTemplateDataValidationError extends Error {
...
const validateCurrentSchemaVersion = (value: unknown, field = 'schemaVersion'): number => {
...
const validateFiniteNumberRecord = (
  value: unknown,
  field: string,
  ...
  for (const [key, item] of Object.entries(record)) {
    if (!key.trim()) fail(field, 'keys must be non-empty strings');
...
interface ValidatedParameterData {
  name: string;
...
interface ValidatedFormulaData {
  name: string;
  expression: string;
  variables: FormulaVariable[];
  expressionNode: FormulaPolicyNode;
}
...
interface ValidatedNode {
  ...
  data: ValidatedParameterData | ValidatedFormulaData | ValidatedOutputData;
  path: string;
}
...
interface ValidatedGraph {
  nodes: ValidatedNode[];
  nodeById: Map<string, ValidatedNode>;
  incomingByTarget: Map<string, ValidatedEdge[]>;
  sortedNodeIds: string[];
  parameterNodes: ValidatedNode[];
  outputNodes: ValidatedNode[];
}
...
const validateFormulaExpression = (
  expression: string,
  ...
  try {
    ...
    if (!candidate) fail(field, 'parser returned an invalid AST');
    parsed = candidate as FormulaPolicyNode;
  } catch (error) {
    if (error instanceof ProfitTemplateDataValidationError) throw error;
    const message = error instanceof Error ? error.message : 'invalid expression';
    fail(field, `has invalid syntax: ${message}`);
  }

  let nodeCount = 0;
  const visit = (node: FormulaPolicyNode, depth: number): void => {
    nodeCount += 1;
    if (nodeCount > GRAPH_EXECUTION_LIMITS.maxFormulaAstNodes) {
      fail(field, `AST must not exceed ${GRAPH_EXECUTION_LIMITS.maxFormulaAstNodes} nodes`);
    }
    if (depth > GRAPH_EXECUTION_LIMITS.maxFormulaAstDepth) {
      fail(field, `AST must not exceed depth ${GRAPH_EXECUTION_LIMITS.maxFormulaAstDepth}`);
    }
    ...
    if (node.type === 'SymbolNode') {
      if (typeof node.name !== 'string') fail(field, 'symbols must have a valid name');
      if (!variableLabels.has(node.name as string) &&
          !ALLOWED_FORMULA_CONSTANTS.has(node.name as string)) {
    ...
    if (node.type === 'ParenthesisNode') {
      ...
      if (!content) fail(field, 'parentheses must contain one scalar expression');
      visit(content as FormulaPolicyNode, depth + 1);
      return;
    }
    if (node.type === 'OperatorNode') {
      ...
      const operatorArgs = node.args as unknown[];
      ...
      for (const argument of operatorArgs) {
        ...
        if (!argumentNode) fail(field, `operator "${node.op}" contains an invalid operand`);
        visit(argumentNode as FormulaPolicyNode, depth + 1);
      }
      return;
    }
    if (node.type === 'FunctionNode') {
      ...
      const directFunctionNode = functionNode as FormulaPolicyNode;
      if (directFunctionNode.type !== 'SymbolNode' || typeof directFunctionNode.name !== 'string') {
        fail(field, 'only direct calls to approved scalar functions are allowed');
      }
      const functionName = directFunctionNode.name as AllowedFormulaFunction;
      const policy = ALLOWED_FORMULA_FUNCTIONS[functionName];
      if (!policy) fail(field, `function "${String(directFunctionNode.name)}" is not allowed`);
      if (!Array.isArray(node.args)) fail(field, `function "${functionName}" has invalid arguments`);
      const functionArgs = node.args as unknown[];
      if (
        functionArgs.length < policy.minArgs ||
        functionArgs.length > policy.maxArgs ||
        functionArgs.length > GRAPH_EXECUTION_LIMITS.maxFunctionArguments
      ) {
        fail(field, `function "${functionName}" received an unsupported number of arguments`);
      }
      for (const argument of functionArgs) {
        const argumentNode = asFormulaPolicyNode(argument);
        if (!argumentNode) fail(field, `function "${functionName}" contains an invalid argument`);
        visit(argumentNode as FormulaPolicyNode, depth + 1);
      }
      return;
    }
...
const validateParameterNode = (
  data: JsonRecord,
  path: string,
): ValidatedParameterData => {
  ...
  const valueType = rawValueType as ValidatedParameterData['valueType'];
  ...
  if (min > max) fail(`${path}.data.min`, 'must not exceed max');
...
const validateFormulaNode = (
  data: JsonRecord,
  path: string,
): ValidatedFormulaData => {
  const name = requireString(data.name, `${path}.data.name`);
  const expression = requireString(data.expression, `${path}.data.expression`);
  if (expression.length > GRAPH_EXECUTION_LIMITS.maxExpressionLength) {
    fail(
      `${path}.data.expression`,
      `must not exceed ${GRAPH_EXECUTION_LIMITS.maxExpressionLength} characters`,
    );
  }
  const variablesValue = data.variables;
  if (!Array.isArray(variablesValue)) fail(`${path}.data.variables`, 'must be an array');
  const variables = variablesValue as unknown[];
  if (variables.length > GRAPH_EXECUTION_LIMITS.maxVariablesPerFormula) {
    fail(
      `${path}.data.variables`,
      `must not exceed ${GRAPH_EXECUTION_LIMITS.maxVariablesPerFormula} variables`,
    );
  }

  const portIds = new Set<string>();
  const labels = new Set<string>();
  const parsedVariables = (variables as unknown[]).map((value, index) => {
    ...
    if (portIds.has(portId)) fail(`${path}.data.variables[${index}].portId`, 'must be unique');
    if (labels.has(label)) fail(`${path}.data.variables[${index}].label`, 'must be unique');
    portIds.add(portId);
    labels.add(label);
    return { portId, label };
  });
...
const validateGraphSnapshot = (
  ...
  if (!Array.isArray(snapshotNodesValue)) fail('graphTemplateSnapshot.nodes', 'must be an array');
  if (!Array.isArray(snapshotEdgesValue)) fail('graphTemplateSnapshot.edges', 'must be an array');
  const snapshotNodes = snapshotNodesValue as unknown[];
  const snapshotEdges = snapshotEdgesValue as unknown[];
  if (snapshotNodes.length > GRAPH_EXECUTION_LIMITS.maxNodes) {
    fail(
      'graphTemplateSnapshot.nodes',
      `must not exceed ${GRAPH_EXECUTION_LIMITS.maxNodes} nodes`,
    );
  }
  if (snapshotEdges.length > GRAPH_EXECUTION_LIMITS.maxEdges) {
    fail(
      'graphTemplateSnapshot.edges',
      `must not exceed ${GRAPH_EXECUTION_LIMITS.maxEdges} edges`,
    );
  }

  const nodeIds = new Set<string>();
  let totalVariables = 0;
  const nodes: ValidatedNode[] = snapshotNodes.map((value, index): ValidatedNode => {
    const path = `graphTemplateSnapshot.nodes[${index}]`;
    const node = requireRecord(value, path);
    const id = requireString(node.id, `${path}.id`);
    if (nodeIds.has(id)) fail('graphTemplateSnapshot.nodes', `duplicate node id ${id}`);
    nodeIds.add(id);
    ...
    const type = rawType as ValidatedNode['type'];
  ...
  if (totalVariables > GRAPH_EXECUTION_LIMITS.maxTotalVariables) {
    fail(
      'graphTemplateSnapshot.nodes',
      `formula variables must not exceed ${GRAPH_EXECUTION_LIMITS.maxTotalVariables} in total`,
    );
  }

  const edgeIds = new Set<string>();
  const incomingByTarget = new Map(nodes.map(node => [node.id, [] as ValidatedEdge[]]));
  const edges: ValidatedEdge[] = snapshotEdges.map((value, index) => {
    const path = `graphTemplateSnapshot.edges[${index}]`;
    const edge = requireRecord(value, path);
    const id = requireString(edge.id, `${path}.id`);
    if (edgeIds.has(id)) fail('graphTemplateSnapshot.edges', `duplicate edge id ${id}`);
    edgeIds.add(id);
    const source = requireString(edge.source, `${path}.source`);
    const target = requireString(edge.target, `${path}.target`);
    if (!nodeIds.has(source)) fail(`${path}.source`, 'must reference an existing node');
    if (!nodeIds.has(target)) fail(`${path}.target`, 'must reference an existing node');
  ...
  for (const node of nodes) {
    ...
    if (node.type === 'formula') {
      const formulaData = node.data as ValidatedFormulaData;
      ...
      for (let index = 0; index < formulaData.variables.length; index += 1) {
        ...
        if (matches.length !== 1) {
          fail(
            `${node.path}.data.variables[${index}]`,
            'must have exactly one matching incoming edge',
          );
        }
      }
      const undeclared = incoming.find(
        edge => !edge.targetHandle || !declaredPorts.has(edge.targetHandle),
      );
      if (undeclared) {
...
const validateRuntimeRecordKeys = (
  ...
  for (const key of Object.keys(record)) {
    if (!expectedIds.has(key)) fail(`${field}.${key}`, 'does not match a graph node');
  }
};

const evaluateValidatedGraph = (
  ...
  for (const nodeId of graph.sortedNodeIds) {
    ...
    if (node.type === 'parameter') {
      const data = node.data as ValidatedParameterData;
      const input = graphInputValues[node.id];
      if (input < data.min || input > data.max) {
        fail(
          `graphInputValues.${node.id}`,
          `must be within ${data.min} and ${data.max}`,
        );
      }
    ...
    if (node.type === 'formula') {
      const data = node.data as ValidatedFormulaData;
      ...
      for (const variable of data.variables) {
        ...
        scope[variable.label] = value as number;
      }
      try {
        ...
        values.set(node.id, result as number);
      } catch (error) {
        if (error instanceof ProfitTemplateDataValidationError) throw error;
    ...
    values.set(node.id, sourceValue as number);
  }
  return values;
};

const outputsMatch = (persisted: number, computed: number): boolean => {
  const tolerance = 1e-10 * Math.max(1, Math.abs(persisted), Math.abs(computed));
  return Math.abs(persisted - computed) <= tolerance;
};

const validateGraphData = (data: JsonRecord): void => {
...
const isGraphClaim = (data: JsonRecord): boolean => (
  data.kind === 'graph' ||
  GRAPH_KEYS.some(key => Object.prototype.hasOwnProperty.call(data, key))
);

const validateInvalidCompatibilityData = (data: JsonRecord): void => {
...
export const validateSharedProfitTemplateData = (value: unknown): Prisma.InputJsonObject => (
  validateTemplateData(value, false) as Prisma.InputJsonObject
);

export const validateProductProfitTemplateData = (value: unknown): Prisma.InputJsonObject => (
  validateTemplateData(value, true) as Prisma.InputJsonObject
);

...
const MODULE_NAME = 'chroma-adapt-v2';

function errorResponse(error: unknown, res: Response): void {
  if (error instanceof ApiError) {
    res.status(error.status_code).json({ detail: error.detail, module: MODULE_NAME, version: MODULE_VERSION });
  } else {
    res.status(500).json({ detail: String(error), module: MODULE_NAME, version: MODULE_VERSION });
  }
}
...
router.get('/', (_req: Request, res: Response) => {
  res.json({ status: 'ok', message: 'ChromaAdapt V2 AI Backend Running', module: MODULE_NAME, version: MODULE_VERSION });
});

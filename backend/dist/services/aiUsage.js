...
function aiRequestHash(input) {
    return (0, crypto_1.createHash)('sha256').update(canonical({ module: input.module || 'chroma', mode: input.mode, model: input.model, kind: input.kind, operationId: input.operationId, payload: input.payload })).digest('hex');
}
async function runAiCall(input, provider) {
    ...
    try {
        ...
        const notSubmitted = (error instanceof config_1.ApiError && error.notSubmitted) || error?.notSubmitted === true;
        ...
        const reason = error instanceof config_1.ApiError
            ? error.detail
            : typeof error?.message === 'string' && error.message
                ? error.message.slice(0, 200)
                : '';
        const baseDetail = failed ? '供应商拒绝请求' : '调用结果未知或无有效产出；请核对调用记录，不要盲目重新生成';
        throw new config_1.ApiError(failed ? 422 : 502, reason ? `${baseDetail}（${reason}）` : baseDetail);
    }
    ...
    const price = input.module === 'product-analysis'
        ...
        : config_1.MODEL_COSTS[input.model];

...
exports.MIN_TOTAL_VISITORS = void 0;
exports.rankPotentialItems = rankPotentialItems;
exports.MIN_TOTAL_VISITORS = 50;
...
function computeMetrics(candidate) {
    ...
    const sumOrders = (rows) => rows.reduce((total, row) => total + (row.ordersOrdered || 0), 0);
...
function rankPotentialItems(candidates, limit = 10) {
    const eligible = candidates.filter((candidate) => !EXCLUDED_STATUS.has(String(candidate.status ?? '')) &&
        candidate.daily.reduce((total, row) => total + (row.visitors || 0), 0) >= exports.MIN_TOTAL_VISITORS);
    ...
    const scored = eligible.map((candidate) => {
        ...
        if (ctrPct >= 60 && cvrPct <= 40) {
            reasons.push(`点击率 ${formatPercent(metrics.ctr)} 但访客转化率仅 ${formatPercent(metrics.cvrConfirmed)}，详情页/价格有优化空间`);
        }
        if (trafficScore >= 70) {

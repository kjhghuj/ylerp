...
exports.ShopeeAuthorizationService = void 0;
exports.startShopeeTokenRefresh = startShopeeTokenRefresh;
...
class ShopeeAuthorizationService {
    db;
    constructor(db) {
        this.db = db;
    }
    async status(userId) {
        let configuration;
        try {
            const config = (0, shopeeClient_1.shopeeConfig)();
            ...
            configuration = { ready: false, error: error instanceof shopeeClient_1.ShopeeError ? error.message : '授权配置不可用。' };
        }
    ...
    async begin(userId) {
        ...
        const session = await this.db.shopeeAuthSession.create({ data: {
                userId, stateHash: (0, shopeeClient_1.secretHash)(state), appBinding: (0, shopeeClient_1.appBinding)(config), expiresAt: new Date(Date.now() + SESSION_MS),
            } });
        const redirect = new URL(config.redirectUrl);
        redirect.searchParams.set('state', state);
        const authorizationUrl = (0, shopeeClient_1.signedShopeeUrl)(config, '/api/v2/shop/auth_partner');
    ...
    async callback(query) {
        ...
        if (!/^[a-f0-9]{64}$/.test(state))
            throw new shopeeClient_1.ShopeeError('授权状态无效，请从 ERP 重新发起授权。');
        const session = await this.db.shopeeAuthSession.findUnique({ where: { stateHash: (0, shopeeClient_1.secretHash)(state) } });
        if (!session || session.status !== 'pending' || session.expiresAt.getTime() <= Date.now()
            || session.appBinding !== (0, shopeeClient_1.appBinding)(config))
            throw new shopeeClient_1.ShopeeError('授权已过期、已使用或应用配置已变化，请重新发起。');
        if (query.error)
            throw new shopeeClient_1.ShopeeError('虾皮未完成授权，请返回 ERP 重试。');
        if (query.main_account_id)
            throw new shopeeClient_1.ShopeeError('当前入口按店铺授权，请选择店铺账号或跨境测试店铺，不要选择主账号授权。');
        if (!code || code.length > 2048 || !/^[1-9]\d*$/.test(shopId) || !Number.isSafeInteger(Number(shopId))) {
            throw new shopeeClient_1.ShopeeError('虾皮回调缺少有效的授权码或店铺 ID。');
        }
        ...
        if (claimed.count !== 1)
            throw new shopeeClient_1.ShopeeError('该授权已在处理中，请勿重复提交。', 409);
        try {
            const data = await (0, shopeeClient_1.shopeeRequest)(config, '/api/v2/auth/token/get', {
                code, partner_id: Number(config.partnerId), shop_id: Number(shopId),
            });
            const tokens = (0, shopeeClient_1.parseShopeeTokens)(data);
            ...
            await this.db.shopeeAuthSession.update({ where: { id: session.id }, data: {
                    status: 'awaiting_confirmation', shopId, pendingCredentials: (0, shopeeClient_1.encryptShopeeCredentials)(tokens, config),
                    confirmationHash: (0, shopeeClient_1.secretHash)(confirmationToken), expiresAt: new Date(Date.now() + SESSION_MS),
                } });
    ...
    async confirm(userId, sessionId, confirmationToken) {
        const config = (0, shopeeClient_1.shopeeConfig)();
        if (!/^[a-f0-9]{64}$/.test(confirmationToken))
            throw new shopeeClient_1.ShopeeError('无效的授权确认。');
        const connection = await this.db.$transaction(async (tx) => {
            const session = await tx.shopeeAuthSession.findFirst({ where: {
                    id: sessionId, userId, status: 'awaiting_confirmation', confirmationHash: (0, shopeeClient_1.secretHash)(confirmationToken),
                    appBinding: (0, shopeeClient_1.appBinding)(config), expiresAt: { gt: new Date() },
                } });
            if (!session?.shopId || !session.pendingCredentials)
                throw new shopeeClient_1.ShopeeError('授权确认无效或过期，请使用发起授权的 ERP 账号。', 403);
            const tokens = (0, shopeeClient_1.decryptShopeeCredentials)(session.pendingCredentials, config);
            const expiresAt = new Date(tokens.expiresAt);
            if (expiresAt.getTime() <= Date.now())
                throw new shopeeClient_1.ShopeeError('授权令牌已过期，请重新授权。');
            ...
            if (claimed.count !== 1)
                throw new shopeeClient_1.ShopeeError('授权已确认。', 409);
            ...
            if (existing && existing.userId !== userId)
                throw new shopeeClient_1.ShopeeError('该店铺已绑定其他 ERP 账号，请联系管理员。', 409);
    ...
    async updateShopInfo(userId, id) {
        const config = (0, shopeeClient_1.shopeeConfig)();
        ...
        if (!connection)
            return;
        const tokens = (0, shopeeClient_1.decryptShopeeCredentials)(connection.encryptedCredentials, config);
        const data = await (0, shopeeClient_1.shopeeRequest)(config, '/api/v2/shop/get_shop_info', undefined, { id: connection.shopId, token: tokens.accessToken });
    ...
    async refresh(userId, id) {
        ...
        const connection = await this.db.shopeeConnection.findFirst({ where: { id, userId, environment: config.environment, partnerId: config.partnerId } });
        if (!connection)
            throw new shopeeClient_1.ShopeeError('店铺不存在或不属于当前应用环境。', 404);
        ...
        if (claimed.count !== 1)
            throw new shopeeClient_1.ShopeeError('该店铺的令牌正在刷新，请稍后再试。', 409);
        try {
            const tokens = (0, shopeeClient_1.decryptShopeeCredentials)(connection.encryptedCredentials, config);
            const data = await (0, shopeeClient_1.shopeeRequest)(config, '/api/v2/auth/access_token/get', {
                partner_id: Number(config.partnerId), shop_id: Number(connection.shopId), refresh_token: tokens.refreshToken,
            });
            const refreshed = (0, shopeeClient_1.parseShopeeTokens)(data);
            ...
            await this.db.shopeeConnection.updateMany({ where: { id, refreshLeaseUntil: lease, encryptedCredentials: connection.encryptedCredentials }, data: {
                    encryptedCredentials: (0, shopeeClient_1.encryptShopeeCredentials)(refreshed, config), expiresAt,
                    nextRefreshAt: new Date(Math.max(Date.now() + 30_000, expiresAt.getTime() - REFRESH_EARLY_MS)),
                    status: 'active', lastError: null, refreshLeaseUntil: null,
                } });
            ...
            const code = error instanceof shopeeClient_1.ShopeeError ? error.code : 'storage_error';
            await this.db.shopeeConnection.updateMany({ where: { id, refreshLeaseUntil: lease, encryptedCredentials: connection.encryptedCredentials }, data: {
    ...
    async refreshDue() {
        let config;
        try {
            config = (0, shopeeClient_1.shopeeConfig)();
        }
        catch {
            return;
        }
        ...
        for (const connection of due)
            await this.refresh(connection.userId, connection.id).catch(() => undefined);
    }
}
exports.ShopeeAuthorizationService = ShopeeAuthorizationService;
function startShopeeTokenRefresh(service) {
    let running = false;
    const timer = setInterval(async () => {
        ...
        catch { /* Retry on the next interval; never log tokens. */ }
        finally {
            running = false;
        }
    }, 60_000);

...
exports.METRIC_LABELS = exports.MAX_PROMPT_CONTEXT_CHARS = void 0;
exports.buildShopAnalysisSystemPrompt = buildShopAnalysisSystemPrompt;
exports.serializeAggregatedItem = serializeAggregatedItem;
exports.serializeAggregatedOverview = serializeAggregatedOverview;
exports.MAX_PROMPT_CONTEXT_CHARS = 24_000;
...
exports.METRIC_LABELS = {
    ...
    createdDays: '上架天数',
    currentPrice: '当前价格',
    uncompetitiveVariations: '无竞争力价格变体数',
    competitiveVariations: '有竞争力价格变体数',
    priceFlag: '价格竞争力标记',
...
const VARIATION_KEYS = [
    ...
    { key: 'modelCode', label: '规格货号' },
    { key: 'modelId', label: '全球商品货号' },
    { key: 'salesOrdered', label: '销售额-已下' },
    { key: 'salesConfirmed', label: '销售额-已确认' },
    { key: 'ordersOrdered', label: '订单-已下' },
    { key: 'ordersConfirmed', label: '订单-已确认' },
    { key: 'unitsOrdered', label: '件数-已下' },
    { key: 'unitsConfirmed', label: '件数-已确认' },
    { key: 'buyersOrdered', label: '买家数-已下' },
    { key: 'buyersConfirmed', label: '买家数-已确认' },
    { key: 'cartVisitors', label: '加购访客' },
    { key: 'cartUnits', label: '加购件数' },
];
const MAX_VARIATIONS_IN_PROMPT = 20;
const MAX_OVERVIEW_ITEMS = 20;
const ITEM_NAME_SNIPPET_LENGTH = 60;
function buildShopAnalysisSystemPrompt(scope) {
    const target = scope.mode === 'item' ? '当前单个商品（含日趋势与全部变体）' : '整店区间汇总数据';
    return [
...
function serializeMetrics(item) {
    const lines = [];
    for (const [key, label] of Object.entries(exports.METRIC_LABELS)) {
...
function serializeVariations(item) {
    const variations = item.variations;
    if (!Array.isArray(variations) || variations.length === 0)
        return [];
    const rows = variations.filter(isRecord);
    const sorted = [...rows].sort((a, b) => (typeof b.unitsOrdered === 'number' ? b.unitsOrdered : 0) -
...
function truncate(text) {
    return text.length > exports.MAX_PROMPT_CONTEXT_CHARS
        ? `${text.slice(0, exports.MAX_PROMPT_CONTEXT_CHARS)}\n（数据过长已截断）`
        : text;
}
...
function serializeAggregatedItem(item, series, variations) {
...
function serializeAggregatedOverview(sheets) {
    const sections = [];
    for (const sheet of sheets) {
        ...
        sections.push(`【类别 ${sheet.sheetKey}】商品数 ${items.length} | 总销售额(已下) ${totals.salesOrdered.toFixed(2)} | ` +

...
router.get('/', async (req, res) => {
    try {
        ...
        const cachedGroups = await index_1.safeRedis.get(cacheKey);
        if (cachedGroups) {
            return res.json(JSON.parse(cachedGroups));
        }
        const groups = await index_1.prisma.skuGroup.findMany({ where: { userId } });
        await index_1.safeRedis.set(cacheKey, JSON.stringify(groups), 'EX', 3600);
        res.json(groups);
    }
    catch (error) {
...
router.post('/', async (req, res) => {
    try {
        const userId = req.user.id;
        const group = await index_1.prisma.skuGroup.create({ data: { ...req.body, userId } });
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await index_1.prisma.skuGroup.delete({ where: { id: req.params.id } });
        await index_1.safeRedis.del(`sku-groups:${userId}`);
        res.status(204).send();
    }
    catch (error) {

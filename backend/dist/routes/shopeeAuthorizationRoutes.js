...
exports.receiveShopeeAuthorization = void 0;
exports.configureShopeeAuthorization = configureShopeeAuthorization;
exports.createShopeeManagementRoutes = createShopeeManagementRoutes;
const crypto_1 = require("crypto");
const express_1 = require("express");
const shopeeClient_1 = require("../services/shopeeClient");
let authorizationService;
function configureShopeeAuthorization(service) { authorizationService = service; }
function sendFailure(res, error) {
    return res.status(error instanceof shopeeClient_1.ShopeeError ? error.status : 503).json({
        error: error instanceof shopeeClient_1.ShopeeError ? error.message : '授权服务暂时不可用，请稍后重试。',
    });
}
const receiveShopeeAuthorization = async (req, res) => {
    ...
    if (!authorizationService)
        return res.status(503).json({ error: '授权服务尚未就绪。' });
    try {
...
exports.receiveShopeeAuthorization = receiveShopeeAuthorization;
function createShopeeManagementRoutes(service) {
    ...
    router.post('/authorize', async (req, res) => {
        try {
            res.json(await service.begin(req.user.id));
        }
        catch (error) {
            sendFailure(res, error);
        }
    ...
    router.post('/confirm', async (req, res) => {
        if (typeof req.body?.sessionId !== 'string' || !/^[a-f0-9-]{36}$/i.test(req.body.sessionId)
            ...
            return res.status(400).json({ error: '授权确认参数无效。' });
        try {
            res.json(await service.confirm(req.user.id, req.body.sessionId, req.body.confirmationToken));
        }
        catch (error) {
            sendFailure(res, error);
        }
    ...
    router.post('/connections/:id/refresh', async (req, res) => {
        ...
        if (typeof id !== 'string' || !/^[a-f0-9-]{36}$/i.test(id))
            return res.status(400).json({ error: '店铺记录无效。' });
        try {
            res.json(await service.refresh(req.user.id, id));
        }
        catch (error) {
            sendFailure(res, error);
        }

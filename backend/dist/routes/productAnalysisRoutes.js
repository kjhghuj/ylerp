...
function parsePageNumber(value, fallback, max) {
    if (value === undefined)
        return fallback;
    ...
    try {
        ...
        if (body?.site !== undefined) {
            ...
            if (nextCurrency !== shop.currency) {
                const existingDays = await index_1.prisma.productAnalysisDailyUpload.count({ where: { shopId: shop.id, isActive: true } });
                if (existingDays > 0) {
...
router.get('/shops/:id/daily-uploads/:date/versions', async (req, res) => {
    try {
        const shop = await findOwnedShop(String(req.params.id ?? ''), req.user.id);
        if (!shop)
            return res.status(404).json({ detail: 'Shop not found' });
        ...
        const versions = await index_1.prisma.productAnalysisDailyUpload.findMany({
...
router.get('/shops/:id/daily-upload-versions/:uploadId/source-sheets', async (req, res) => {
    try {
        const shop = await findOwnedShop(String(req.params.id ?? ''), req.user.id);
        if (!shop)
            return res.status(404).json({ detail: 'Shop not found' });
        const uploadId = String(req.params.uploadId ?? '');
        const upload = await index_1.prisma.productAnalysisDailyUpload.findFirst({
            where: { id: uploadId, shopId: shop.id },
            select: { id: true, version: true, date: true, sourceComplete: true },
        });
        if (!upload)
            return res.status(404).json({ detail: 'Upload version not found' });
        const sheets = await index_1.prisma.productAnalysisSourceSheet.findMany({
            where: { uploadId },
...
router.get('/shops/:id/daily-upload-versions/:uploadId/source-sheets/:sheetIndex', async (req, res) => {
    try {
        const shop = await findOwnedShop(String(req.params.id ?? ''), req.user.id);
        if (!shop)
            return res.status(404).json({ detail: 'Shop not found' });
        ...
        const sheet = await index_1.prisma.productAnalysisSourceSheet.findFirst({
        ...
        if (!sheet)
            return res.status(404).json({ detail: 'Source sheet not found' });
...
router.get('/shops/:id/daily-upload-versions/:uploadId/items', async (req, res) => {
    try {
        const shop = await findOwnedShop(String(req.params.id ?? ''), req.user.id);
        if (!shop)
            return res.status(404).json({ detail: 'Shop not found' });
        ...
        const [total, items] = await Promise.all([
            index_1.prisma.productDailyItem.count({ where: { uploadId } }),
            index_1.prisma.productDailyItem.findMany({ where: { uploadId }, orderBy: { itemId: 'asc' }, skip: offset, take: limit }),
        ]);
        return res.json({
...
router.post('/shops/:id/daily-upload-versions/:uploadId/activate', requireProductAnalysisPermission('product-analysis.upload'), async (req, res) => {
    try {
        const shop = await findOwnedShop(String(req.params.id ?? ''), req.user.id);
        if (!shop)
            return res.status(404).json({ detail: 'Shop not found' });
        const uploadId = String(req.params.uploadId ?? '');
        const target = await index_1.prisma.productAnalysisDailyUpload.findFirst({
            where: { id: uploadId, shopId: shop.id },
        ...
        if (!target)
            return res.status(404).json({ detail: 'Upload version not found' });
        if (!target.isActive) {
            await (0, usageEvents_1.withUsageEvent)(index_1.prisma, req, {
                ...
                if (activated.count !== 1)
                    throw new ProductAnalysisNotFoundError('Upload version not found');
                return activated;
...
router.post('/shops/:id/daily-uploads', requireProductAnalysisPermission('product-analysis.upload'), async (req, res) => {
    try {
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
            try {
                created = await (0, usageEvents_1.withUsageEvent)(index_1.prisma, req, {
                ...
                if (attempt === 2 || !isRetryableSerializationError(error))
                    throw error;
            }
        }
        if (!created)
...
router.post('/chat', requireProductAnalysisPermission('product-analysis.aiChat'), async (req, res) => {
    try {
        ...
        if (itemId) {
            ...
            const detail = (0, productAnalysisAggregation_1.buildItemDetail)(rows);
            const context = (0, prompts_1.serializeAggregatedItem)({ ...detail.item, ...(detail.extra ?? {}) }, detail.series, detail.variations);

...
import { ChromaAppState, ProcessingState, PipelineItem } from '../chromaTypes';
...
export const useFileHandlers = ({ state, setState, performAnalysis }: UseFileHandlersProps) => {
        ...
        setState(prev => {
          ...
          if (isPoster) {
             ...
             return {
               ...
               resultImage: null, 
               editPrompt: '', // Clear prompt on new upload to trigger auto-analysis
               status: nextStatus
             };
          } else {
             return {
  ...
  const handlePipelineBatchUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Explicitly cast to File[] to ensure TS treats items as Files/Blobs, fixing 'unknown' type error

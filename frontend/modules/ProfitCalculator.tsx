...
const DEFAULT_NODE_DATA = {
    totalRevenue: 0, baseShippingFee: 0, extraShippingFee: 0, crossBorderFee: 0,
...
export const ProfitCalculator: React.FC = () => {
    ...
    const [globalInputs, setGlobalInputs] = useState({
        name: '', sku: '', purchaseCost: 0, productWeight: 0, firstWeight: 50,
        supplierTaxPoint: 0, supplierInvoice: 'yes',
    });
    ...
    const [selectedCountry, setSelectedCountry] = useState('MYR');
    ...
    const handleSaveProduct = async () => {
        if (!globalInputs.name || !globalInputs.sku) {
            alert('Please enter Name and SKU');
            return;
        }
        ...
        const productData: Omit<ProductCalcData, 'id'> = {
            ...
            totalRevenue: Number(node.data.totalRevenue) || 0,
            ...
            sellerCoupon: Number(node.data.sellerCoupon) || 0,
            ...
            adROI: Number(node.data.adROI) || 0,
            ...
            platformInfrastructureFee: Number(node.data.platformInfrastructureFee) || 0,
            warehouseOperationFee: Number(node.data.warehouseOperationFee) || 0,
            vatRate: Number(node.data.vatRate) || 0,
            corporateIncomeTaxRate: Number(node.data.corporateIncomeTaxRate) || 0,
        ...
        alert(editingProductId ? t.actions.saved : t.actions.saved);
    };
        ...
        <div className="flex flex-col min-h-[calc(100vh-140px)] pb-6">
                        ...
                        {showAddMenu && (
                            <div className="absolute top-full right-0 mt-2 w-72 bg-white rounded-2xl shadow-xl border border-slate-100 p-4 z-50">
                                ...
                                <div className="space-y-1 mb-4 max-h-48 overflow-y-auto">
                                    {allTemplates.map(tpl => (
                                        <div key={tpl.id} className="flex justify-between items-center p-2 hover:bg-slate-50 rounded-lg cursor-pointer border border-transparent hover:border-slate-200" onClick={() => handleAddNodeFromTemplate(tpl)}>
                                            <div>
                                                ...
                                                <div className="text-[10px] text-slate-400 capitalize">{t.matrix.platforms[tpl.platform || 'other'] || tpl.platform} • {tpl.country}</div>
                                            </div>
                                    ...
                                    {allTemplates.length === 0 && <p className="text-xs text-slate-400 text-center py-2">{t.templates.empty || '暂无模版'}</p>}
                                </div>
                                    ...
                                    <div className="flex gap-2 mb-3">
                                        <div className="flex-1 relative group">
                                            <select 
                                                ...
                                                className="w-full text-[11px] font-bold p-2.5 pr-8 bg-slate-50 border border-slate-200 rounded-xl outline-none appearance-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all cursor-pointer shadow-sm hover:bg-slate-100/50"
                                            >
                                                {Object.entries(t.matrix.sites || {}).map(([code, name]: [any, any]) => (
                                                    <option key={code} value={code} className="font-sans py-2">{name}</option>
                                                ))}
                                            </select>
            ...
            <div className="bg-white/70 backdrop-blur-xl border border-white/50 shadow-sm rounded-xl p-4 mb-4 flex-shrink-0">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                    ...
                    <NumberInput label={`${t.inputs.cost} (CNY)`} name="purchaseCost" value={globalInputs.purchaseCost} onChange={handleGlobalChange} highlight />
